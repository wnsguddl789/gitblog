...
export const fileOrDirExists = (pathLike) => {
    return (OT.withSpan('fileOrDirExists', { attributes: { pathLike } })(T.tap_(T.catchTag_(T.map_(stat(pathLike), (stat_) => stat_.isFile() || stat_.isDirectory(), fileName_1 + ":14:10"), 'node.fs.FileNotFoundError', () => T.succeed(false, fileName_1 + ":15:60"), fileName_1 + ":15:15"), (exists) => OT.addAttribute('exists', exists), fileName_1 + ":16:10")));
};
export const symlinkExists = (pathLike) => {
    return (T.catchTag_(T.map_(stat(pathLike), (stat_) => stat_.isSymbolicLink(), fileName_1 + ":24:10"), 'node.fs.FileNotFoundError', () => T.succeed(false, fileName_1 + ":25:60"), fileName_1 + ":25:15"));
};
...
export const readFileJson = (filePath) => (T.chain_(readFile(filePath), (str) => T.tryCatch(() => JSON.parse(str), (error) => new JsonParseError({ str, error }), fileName_1 + ":62:17"), fileName_1 + ":61:12"));
export const readFileJsonIfExists = (filePath) => (T.catchTag_(T.chain_(fileOrDirExists(filePath), (exists) => (exists ? readFileJson(filePath) : T.succeed(undefined, fileName_1 + ":74:72")), fileName_1 + ":74:12"), 'node.fs.FileNotFoundError', (e) => T.die(e, fileName_1 + ":75:57"), fileName_1 + ":75:15"));
export const writeFile = (filePath, content) => OT.withSpan('writeFile', { attributes: { filePath } })(T.tryCatchPromise(() => fs.writeFile(filePath, content, 'utf8'), (error) => new WriteFileError({ filePath, error }), fileName_1 + ":80:22"));
export const writeFileJson = ({ filePath, content, }) => (T.chain_(T.tryCatch(() => JSON.stringify(content, null, 2) + '\n', (error) => new JsonStringifyError({ error }), fileName_1 + ":94:15"), (contentStr) => writeFile(filePath, contentStr), fileName_1 + ":98:12"));
export const mkdirp = (dirPath) => OT.withSpan('mkdirp', { attributes: { dirPath } })(T.tryCatchPromise(() => fs.mkdir(dirPath, { recursive: true }), (error) => new MkdirError({ dirPath, error }), fileName_1 + ":103:22"));
 ...
 */
export const symlink = ({ targetPath, symlinkPath, type, }) => OT.withSpan('symlink', { attributes: { targetPath, symlinkPath, type } })(T.tryCatchPromise(() => fs.symlink(targetPath, symlinkPath, type), (error) => new SymlinkError({ targetPath, symlinkPath, type, error }), fileName_1 + ":149:22"));
