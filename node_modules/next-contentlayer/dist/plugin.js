...
export const runContentlayerDev = async () => {
    ...
    await (runMain(S.runDrain(S.tap_(S.chainSwitchMapEitherRight((config) => core.generateDotpkgStream({ config, verbose: false, isDev: true }))(S.tapRight((config) => (config.source.options.disableImportAliasWarning ? T.unit : T.fork(core.validateTsconfig, fileName_1 + ":17:94")))(S.tapSkipFirstRight(() => T.log(`Contentlayer config change detected. Updating type definitions and data...`))(core.getConfigWatch({})))), E.fold((error) => T.log(errorToString(error)), core.logGenerateInfo)))));
};
export const runContentlayerBuild = async () => {
    if (contentlayerInitialized)
        return;
    contentlayerInitialized = true;
    await (runMain(OT.withSpan('next-contentlayer:runContentlayerBuild')(T.tap_(T.chain_(core.getConfig({}), (config) => core.generateDotpkg({ config, verbose: false }), fileName_1 + ":31:12"), core.logGenerateInfo, fileName_1 + ":32:10"))));
};
