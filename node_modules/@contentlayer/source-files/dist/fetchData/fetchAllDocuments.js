...
export const fetchAllDocuments = ({ coreSchemaDef, filePathPatternMap, contentDirPath, contentDirInclude, contentDirExclude, contentTypeMap, flags, options, previousCache, verbose, }) => (OT.withSpan('@contentlayer/source-local/fetchData:fetchAllDocuments', { attributes: { contentDirPath } })(provideDocumentTypeMapState(T.gen(function* ($) {
    const allRelativeFilePaths = yield* $(getAllRelativeFilePaths({ contentDirPath, contentDirInclude, contentDirExclude }), fileName_1 + ":42:44");
    ...
    const { dataErrors, documents } = yield* $((T.map_(T.map_(T.forEachParN_(allRelativeFilePaths, concurrencyLimit, (relativeFilePath) => makeCacheItemFromFilePath({
        ...
        previousCache,
        contentTypeMap,
    }), fileName_1 + ":51:24"), Chunk.partitionThese, fileName_1 + ":62:16"), ({ tuple: [errors, docs] }) => ({ dataErrors: Chunk.toArray(errors), documents: Chunk.toArray(docs) }), fileName_1 + ":63:16")), fileName_1 + ":48:49");
    const singletonDataErrors = yield* $(validateSingletonDocuments({ coreSchemaDef, filePathPatternMap }), fileName_1 + ":67:43");
...
const getAllRelativeFilePaths = ({ contentDirPath, contentDirInclude, contentDirExclude, }) => {
    ...
    return (OT.withSpan('@contentlayer/source-local/fetchData:getAllRelativeFilePaths')(T.map_(T.tryCatchPromise(() => glob(pattern, { cwd: contentDirPath, ignore: asMutableArray(contentDirExclude) }), (error) => new fs.UnknownFSError({ error }), fileName_1 + ":113:22"), (_) => _.map(posixFilePath), fileName_1 + ":117:10")));
};
const validateSingletonDocuments = ({ coreSchemaDef, filePathPatternMap, }) => T.gen(function* ($) {
    ...
    const documentTypeMap = yield* $(DocumentTypeMapState.get, fileName_1 + ":134:37");
